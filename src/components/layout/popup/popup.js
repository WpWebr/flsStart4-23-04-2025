...
class Popup {
  constructor(options) {
    let config = {
      ...
      init: true,
      // Для кнопок
      attributeOpenButton: 'data-fls-popup-link', // Атрибут для кнопки, которая вызывает попап
      attributeCloseButton: 'data-fls-popup-close', // Атрибут для кнопки, которая закрывает попап
      // Для сторонних объектов
      fixElementSelector: '[data-fls-lp]', // Атрибут для элементов с левым паддингом (которые fixed)
      // Для объекта попапа
      attributeMain: 'data-fls-popup',
      youtubeAttribute: 'data-fls-popup-youtube', // Атрибут для кода youtube
      ...
      classes: {
        ...
        popupActive: 'data-fls-popup-active', // Добавляется для попапа, когда он открывается
        bodyActive: 'data-fls-popup-open', // Добавляется для body, когда попап открыт
      },
      focusCatch: true, // Фокус внутри попапа зациклен
      closeEsc: true, // Закрытие ESC
      bodyLock: true, // Блокировка скролла
      hashSettings: {
        location: true, // Хеш в адресной строке
        goHash: true, // Переход при наличии в адресной строке
      },
      on: { // События
        beforeOpen: function () { },
        afterOpen: function () { },
  ...
  eventsPopup() {
    ...
    document.addEventListener("click", function (e) {
      ...
      // Закрытие при клике по пустому месту (popup__wrapper) и по кнопке закрытия (popup__close)
  ...
  open(selectorValue) {
    if (bodyLockStatus) {
      ...
      // Если передано значение селектора (селектор настраивается в options)
      ...
      if (this.targetOpen.element) {
        ...
        if (this.options.hashSettings.location) {
          // Получение хеша и его установка
          this._getHash()
          this._setHash()
        }
        // До открытия
        ...
        // Запоминаем это открытое окно. Оно будет последним открытым
