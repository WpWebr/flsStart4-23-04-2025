...
// Настройки
Для селекта (select):
class="имя класса" - модификатор для конкретного селекта
multiple - мультивыбор
data-fls-select-modif= "имя модификатора"
data-fls-select-tags - режим тегов, только для multiple
data-fls-select-scroll - включить прокрутку для выпадающего списка, дополнительно можно подключить кастомный скролл simplebar в app.js. Указанное число для атрибута ограничит высоту
data-fls-select-checkbox - стилизация элементов под checkbox (только для multiple)
data-fls-select-show-selected - отключает скрытие выбранного элемента
data-fls-select-search - позволяет искать по выпадающему списку
data-fls-select-open - селект открыт сразу
data-fls-select-submit - отправляет форму при изменении селекта

data-fls-select-one-select - селекты внутри оболочки с атрибутом показываются только по одному
data-fls-select-pseudo-label - добавляет псевдоэлемент к заголовку селекта с указанным текстом

Для плейсхолдера (Плейсхолдер - это option с value=""):
data-fls-select-label для плейсхолдера, добавляет label к селекту
data-fls-select-show для плейсхолдера, показывает его в списке (только для одиночного выбора)

Для элемента (option):
data-fls-select-class="имя класса" - добавляет класс
data-fls-select-asset="путь к картинке или текст" - добавляет структуру 2-х колонок с данными
data-fls-select-href="адрес ссылки" - добавляет ссылку в элемент списка
data-fls-select-href-blank - откроет ссылку в новом окне
*/

/*
// Возможные доработки:
попап на мобильной версии
обновление данных (Женя iniline)
*/
...
class SelectConstructor {
  constructor(props, data = null) {
    ...
    this.selectClasses = {
      ...
      classSelectValue: "select__value", // Значення в заголовке
      classSelectLabel: "select__label", // Лабел
      classSelectInput: "select__input", // Поле ввода
      classSelectText: "select__text", // Оболочка текстових даних
  ...
  selectAction(selectItem) {
    ...
    // Определяем, где отображать выпадающий список
  ...
  getSelectElementContent(selectOption) {
    // Если для элемента указаны картинка или текст, перестраиваем конструкцию
